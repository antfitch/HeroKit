...
namespace HeroKit.Scene.Actions
{
    ...
    /// </summary>
    public class MoveTowardObject2D : IHeroKitAction
    {
        ...
        public static MoveTowardObject2D Create()
        {
            MoveTowardObject2D action = new MoveTowardObject2D();
            return action;
        }
        ...
        {
            // exit early if target or object to avoid are missing
            if (targetObject == null || avoidObject == null) return;

            // get the movement script
            moveObject = targetObject.GetHeroComponent<RpgMovement2D>("RpgMovement2D", true);
            moveObject.targetObject = avoidObject;
