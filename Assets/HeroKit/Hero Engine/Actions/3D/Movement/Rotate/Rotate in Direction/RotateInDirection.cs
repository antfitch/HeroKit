...
namespace HeroKit.Scene.Actions
{
    ...
    /// </summary>
    public class RotateInDirection : IHeroKitAction
    {
        ...
        public static RotateInDirection Create()
        {
            RotateInDirection action = new RotateInDirection();
            return action;
        }
        ...
        public int Execute(HeroKitObject hko)
        {
            ...
            if (changeX) degrees.x = DropDownListValue.GetValue(heroKitObject, 3);
            bool changeY = BoolValue.GetValue(heroKitObject, 4);
            if (changeY) degrees.y = DropDownListValue.GetValue(heroKitObject, 5);
            bool changeZ = BoolValue.GetValue(heroKitObject, 6);
            if (changeZ) degrees.z = DropDownListValue.GetValue(heroKitObject, 7);
